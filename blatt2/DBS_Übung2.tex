...
\title{\vspace*{-10mm} Datenbanksysteme SoSe 18\\Übung 2}
...
\maketitle
\section{Grundlagen}
\subsection{Geben Sie im ER-Modell ein Beispiel für einen Entitätstypen mit genau einem Attribut an.}
...
\subsection{Geben Sie im ER-Modell ein Beispiel für eine nicht-rekursive 1-zu-N Relation an.}
...
\subsection{Geben Sie im ER-Modell ein Beispiel für eine nicht-rekursive N-zu-M Relation an.}
...
\subsection{Geben Sie im ER-Modell ein Beispiel für eine rekursive Relation an. Wie sieht es mit dem Kardinalitätsverhältnis Ihres Beispiels aus?}
...
\subsection{Erklären Sie den Unterschied zwischen totaler und partieller Partizipation.}
\textbf{Totale Partizipation:} \\
Jede Instanz einer Entität der ersten Klasse muss mit einer Instanz der Entität der zweiten Klasse in Relation stehen. \\
\textbf{Partielle Partizipation:} \\
Eine Instanz einer Entität  kann(!) in Relation zu einer Instanz der zweiten Klasse stehen.

\subsection{Erstellen Sie ein ER-Modell in einfacher Notation auf Grundlage der folgenden Beschreibung. \\
Versuchen Sie sich so nah wie möglich an die Beschreibung zu halten. \\
Beachten Sie bei der Modellierung die Aspekte der totalen und partiellen Abhängigkeit.}
\textbf{Beschreibung:}\\
Ärzte behandeln Patienten. Ärzte haben einen Namen und eine Spezialisierung. Patienten haben einen Namen, eine Versicherungsnummer und leiden an mehreren Krankheiten. Eine Krankheit wird eindeutig durch einen Namen beschrieben. Ärzte dürfen sich nicht selbst behandeln. Patienten tauschen sich mit anderen Patienten über ihre Krankheiten aus. Ärzte forschen daran Krankheiten besser heilen zu können. \\

\includegraphics[scale=0.4]{ER1.png}


\subsection{Erstellen Sie ein ER-Modell in Min-Max Notation auf Grundlage der folgenden Beschreibung. Versuchen Sie sich so nah wie möglich an die Beschreibung zu halten.}
\textbf{Beschreibung:} \\
...
\begin{enumerate}
	...
	\item Port in /etc/apache2/ports.conf zu 8050 ändern
	\item Port in /etc/apache2/sites-enabled/000-default.conf zu *:8050 ändern
	\item index.html in /var/www/html/ ersetzen
	\item apache2 starten: {\lstinline[basicstyle=\ttfamily]|service apache2 start|}
\end{enumerate}

\subsection{Schreiben sie eine Javascript-Funktion, die im div Element bereich den Text "Hallo ", ergänzt	durch den Text aus dem Eingabefeld feld ausgibt, sobald der Knopf knopf gedrückt wird. Wenn kein Text im Eingabefeld feld steht soll "Hallo Unbekannter!" im div Element bereich ausgegeben werden}
